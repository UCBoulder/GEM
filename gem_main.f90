...
!     call balloon
  ...
  do  timestep=ncurr,nm
     tcurr = tcurr+dt
     if((iput.eq.1) .and. (timestep-ncurr)>1 .and. mod(timestep,1000).eq.0)call restart(2,timestep-1)
     ...
     split_start_tm=split_start_tm+mpi_wtime()
     call split_weight(timestep-1,0)
     split_end_tm=split_end_tm+mpi_wtime()
  ...
  dum1=initpmove_end_tm-initpmove_start_tm 
  dum2=pmove_end_tm-pmove_start_tm
  dum3=split_end_tm-split_start_tm   

  call MPI_REDUCE(tottm,total_tm,1,MPI_REAL8,MPI_SUM,0,MPI_COMM_WORLD,ierr)  
  call MPI_REDUCE(dum1,initpmove_tm,1,MPI_REAL8,MPI_SUM,0,MPI_COMM_WORLD,ierr)
  call MPI_REDUCE(dum2,pmove_tm,1,MPI_REAL8,MPI_SUM,0,MPI_COMM_WORLD,ierr)
  call MPI_REDUCE(dum3,split_tm,1,MPI_REAL8,MPI_SUM,0,MPI_COMM_WORLD,ierr)    
  call MPI_REDUCE(push_tot_tm,push_tm,1,MPI_REAL8,MPI_SUM,0,MPI_COMM_WORLD,ierr)
  ...
  initpmove_tm=initpmove_tm/numprocs  
  grid1_tm=grid1_tm/numprocs
  split_tm=split_tm/numprocs  
  push_tm=push_tm/numprocs
  push_tm=push_tm-pmove_tm-initpmove_tm
  
  dum1 = poisson_end_tm-poisson_start_tm
  dum2 = ampere_end_tm-ampere_start_tm
  dum3 = dpdt_end_tm-dpdt_start_tm  
  call MPI_REDUCE(dum1,poisson_tm,1,MPI_REAL8,MPI_SUM,0,MPI_COMM_WORLD,ierr)  
  call MPI_REDUCE(dum2,ampere_tm,1,MPI_REAL8,MPI_SUM,0,MPI_COMM_WORLD,ierr)
  call MPI_REDUCE(dum3,dpdt_tm,1,MPI_REAL8,MPI_SUM,0,MPI_COMM_WORLD,ierr)
  poisson_tm = poisson_tm/numprocs
  ampere_tm = ampere_tm/numprocs
  dpdt_tm = dpdt_tm/numprocs

   if(myid==0)write(*,*)'right caculation'
   if(myid==0)write(*,*)'total_tm',total_tm,'push_tm',push_tm,'initpmove_tm',initpmove_tm,'pmove_tm',pmove_tm,'grid1_tm',grid1_tm,  &
        'poisson_tm',poisson_tm,'ampere_tm', ampere_tm, 'dpdt_tm', dpdt_tm, 'split_tm',split_tm
...
n_omp = nthreads
  ...
  namelist /primary_parameters/ itube,mimp,mcmp,chgi,chgc,imx,jmx,kmx,nsmx,ntube,lxa, &
                                lymult,jcnt,dt,nm,nsm,amp,r0a,ifluid,ifullfbm,cn0b,ipbm,ias,isg,amie,rneui, &
                                betai,nonlin1,nonlin2,nonline,nonlinb,iflut,iexb,ipara,vcut,wecut,micell,mecell, &
                                qbeam,mbeam,isonew
  namelist /control_parameters/ iperi,iperidf,delra,delri,delre,delrn,nlow,xshape, &
                                yshape,zshape,iput,iget,igetmx,ision,isbeam,isiap,peritr,llk,mlk,onemd,izonal, &
...
!  beta=betai 
  ...
  if(myid.eq.master)then
     ...
     write(19,*)'ibmst,ibmend,ibmcnl=', ibmst,ibmend,ibmcnl
...
!  !$acc update device(apar)
  ...
  if(myid.eq.master)then
     ...
     write(9,*)'amp = ',amp
     write(9,*)'isbeam,cn0b,ineq0= ',isbeam,cn0b,ineq0
     ...
     write(9,*)'tot mmi,tot mme,tot mmb= ',mm1,mm2,tmmb
     ...
     write(9,*) 'n_omp,nzcrt,nzsrc=',n_omp,nzcrt,nzsrc      
     close(9)
  end if
...
subroutine ppush(n,ns)
  ...
  real :: dbdrp,dbdtp,grcgtp,bfldp,fp,radiusp,dydrp,qhatp,psipp,jfnp,grdgtp,omrotp
...
!$omp private(grdgtp,fp,jfnp,psipp,psp,ter,kaptp,kapnp,xnp,vncp,vparspp,psip2p,dipdrp,b,pzp,rhog,rhox,rhoy,omrotp) &
...
!$omp shared(x2,y2,z2,u2,w2,mu,x3,y3,z3,u3,w3,g0i,eki,pzi,xii,z0i,u0i,gmrkr,avwixeps,avwixez,dnisrc,dnisrcz,fisrc,dtiz) &
!$omp shared(ex,ey,ez,delbx,delby,apar,phi,dpdz,dadz,delbxh,delbyh,dahdz,aparh) &
!$omp shared(thfnz,dbdr,dbdth,grcgt,bfld,radius,dydr,qhat,gr,gxdgy,curvbz,bdcrvb,grdgt,f,sf,jfn,psip,psi,t0s,capts,capns,xn0s) &
!$omp shared(phincp,vparsp,psip2,dipdr,omrot) &
!$omp shared(lr0,r0,rin,pi,q,br0,tor,q0,lx,ly,lz,dx,dy,dz,delz,dr,dth,dxsrc,desrc,nesrc,nxsrc,nr,gclr,tclr,kcnt,vexbsw,vparsw) &
!$omp shared(ipbm,itube,peritr,iperidf,isonew,gammah,gamgtc,ghzon,gamtoy,gamgyro) &
!$omp shared(mm,ns,mims,iflr,iorb,lr,nonlin,dt,ildu,tgis,ig0,igmrkr,pzcrit) &
!$omp shared(nr2,q0p) &
...
!$acc loop gang vector private(rhox,rhoy) 
  ...
  do m=1,mm(ns)
     ...
     omrotp = wx0*omrot(i)+wx1*omrot(i+1)             
     b=1.-tor+tor*bfldp
...
!$acc loop seq
     ...
     vp0 = r0/q0*omrotp*vexbsw !1./b**2*lr0/q0*qhatp*fp/radiusp*grcgtp
...
333  continue
     ...
     wx0 = (rin+(i+1)*dr-r)/dr
     wx1 = 1.-wx0
     if(itube==0)qr = wx0*sf(i)+wx1*sf(i+1)
     if(itube==1)qr = q0+q0p*(r-lr0)
...
!$acc end parallel
  ...
  if(idg==1)then
     do i = 0,last
        if(myid==i)then
           open(9, file='plot',status='unknown',position='append')
           write(9,*)'before init_pmove in ppush',myid
           close(9)
        end if
        call MPI_BARRIER(MPI_COMM_WORLD,ierr)
     end do
...
ppush1_end_tm=ppush1_end_tm+MPI_WTIME()
  ...
  if (ierr.ne.0) call ppexit
  call test_pmove(g0i(:,ns),np_old,np_new,ierr)
  if (ierr.ne.0) call ppexit
...
subroutine cpush(n,ns)
  ...
  implicit none
  INTEGER :: n
  real :: phip,exp1,eyp,ezp,delbxp,delbyp,dpdzp,dadzp,aparp,eykyp(100),dbxkyp(100)
  real :: wx0,wx1,wy0,wy1,wz0,wz1,w3old,z3old
  INTEGER :: m,i,j,k,ns,l,mynowi,k1
  INTEGER :: np_old,np_new
  ...
  real :: myke,mypfl_es(1:nsubd,1:ntor),mypfl_em(1:nsubd,1:ntor),myavewi
  real :: myefl_es(1:nsubd,1:ntor),myefl_em(1:nsubd,1:ntor),mynos
  real :: ketemp,pfltemp
  real :: efltemp,nostemp
  real :: sbufx(nsubd,ntor),rbufx(nsubd,ntor),sbuf(10),rbuf(10)
  real :: dbdrp,dbdtp,grcgtp,bfldp,fp,radiusp,dydrp,qhatp,psipp,jfnp,grdgtp,omrotp
  real :: grp,gxdgyp,rhox(4),rhoy(4),psp,pzp,vncp,vparspp,psip2p,bdcrvbp,curvbzp,dipdrp
  real :: fdum,gdum,fisrcp,dnisrcp,avwixepsp,fovg,avwixezp,dnisrczp
  !MVPT
...
!$omp parallel do &
!$omp default(none) &
!$omp private(i,j,k,k1,l,r,th,wx0,wx1,wy0,wy1,wz0,wz1,dbdrp,dbdtp,grcgtp,bfldp,radiusp,dydrp,qhatp,grp,gxdgyp,curvbzp,bdcrvbp,grdgtp) &
!$omp private(fp,jfnp,psipp,psp,ter,kaptp,kapnp,xnp,b,psip2p,dipdrp,pzp,vncp,vparspp,rhog,rhox,rhoy,omrotp) &
!$omp private(phip,exp1,eyp,ezp,delbxp,delbyp,dpdzp,dadzp,aparp,xs,xt,yt,vfac,vp0,vpar,bstar,enerb,xdot,ydot,zdot) &
!$omp private(pzd0,pzdot,edot,dum,dum1,vxdum,kap,w3old,z3old,laps,qr,eykyp,dbxkyp) &
!$omp private(fdum,gdum,fisrcp,dnisrcp,avwixepsp,fovg,dtp,avwixezp,dnisrczp) &
!$omp private(dapsdtp,delbxhp,delbyhp,dahdzp,aparhp,zdot0,zdot1) &    !MVPT

!$omp shared(x2,y2,z2,u2,w2,mu,x3,y3,z3,u3,w3,g0i,eki,pzi,xii,z0i,u0i,gmrkr,avwixeps,avwixez,dnisrc,dnisrcz,fisrc,dtiz) &
!$omp shared(ex,ey,ez,delbx,delby,apar,phi,dpdz,dadz,delbxh,delbyh,dahdz,aparh,eyky,dbxky) &
!$omp shared(thfnz,dbdr,dbdth,grcgt,bfld,radius,dydr,qhat,gr,gxdgy,curvbz,bdcrvb,grdgt,f,sf,jfn,psip,psi,t0s,capts,capns,xn0s) &
!$omp shared(phincp,vparsp,psip2,dipdr,omrot) &
!$omp shared(lr0,r0,rin,pi,q,br0,tor,q0,lx,ly,lz,dx,dy,dz,delz,dr,dth,dxsrc,desrc,nesrc,nxsrc,nr,gclr,tclr,kcnt,vexbsw,vparsw) &
!$omp shared(ipbm,itube,peritr,iperidf,isonew,gammah,gamgtc,ghzon,gamtoy,gamgyro,nsubd) &
!$omp shared(mm,ns,mims,iflr,iorb,lr,nonlin,dt,ildu,tgis,ig0,igmrkr,pzcrit,ntor) &
!$omp shared(nr2,q0p) &
...
#ifdef OPENACC
!$acc parallel
!$acc loop gang vector private(eykyp,dbxkyp,rhox,rhoy) 
#endif
  do m=1,mm(ns)
     r=x3(m,ns)-0.5*lx+lr0
...
!$acc loop seq
...
#endif        
        do k1 = 1,ntor
        eykyp(k1) = eykyp(k1)+eyky(i,j,k,k1)
        dbxkyp(k1) = dbxkyp(k1)+dbxky(i,j,k,k1)
        end do
     enddo
     ...
     dadzp = dadzp/4.
     aparp = aparp/4.
     eykyp(1:ntor) = eykyp(1:ntor)/4.
     dbxkyp(1:ntor) = dbxkyp(1:ntor)/4.
     ...
     vxdum = (eyp/b+vpar/b*delbxp)*dum1
     w3old = w3(m,ns)
     w3(m,ns) = w2(m,ns) + dt*(vxdum*kap+edot/ter)*(fovg-w3(m,ns)*nonlin(ns)*isonew) - dt*gammah*(avwixepsp-dnisrcp/(fisrcp+1e-8)*fovg)  &
...
!        w2(ns,m) = 0.
     ...
     do l = 1,ntor 
     !$acc atomic
     mypfl_es(k,l)=mypfl_es(k,l) + w3old*(eykyp(l)/b)*dum1/grp
     !$acc atomic     
     mypfl_em(k,l)=mypfl_em(k,l) + w3old*(vpar*dbxkyp(l)/b)*dum1/grp
     !$acc atomic     
     myefl_es(k,l)=myefl_es(k,l) + vfac*w3old*(eykyp(l)/b)*dum1/grp
     !$acc atomic     
     myefl_em(k,l)=myefl_em(k,l) + vfac*w3old*(vpar*dbxkyp(l)/b)*dum1/grp
     end do
     !$acc atomic     
...
cpush1_end_tm=cpush1_end_tm+MPI_WTIME()
  ...
  sbuf(1)=myke
  sbuf(2)=myefl_es(nsubd/2,ntor)
  sbuf(3)=mypfl_es(nsubd/2,ntor)
  sbuf(4)=mynos
  sbuf(5)=myavewi
  ...
  sbufx(1:nsubd,1:ntor) = myefl_es(1:nsubd,1:ntor)
  call MPI_ALLREDUCE(sbufx,rbufx,nsubd*ntor,  &
       MPI_REAL8,MPI_SUM,  &
       MPI_COMM_WORLD,ierr)
  do k = 1,nsubd
     efl_es(ns,k,:)=rbufx(k,:)/( float(tmm(1)) )*totvol/vol(k)*cn0s(ns)
  end do

  sbufx(1:nsubd,1:ntor) = myefl_em(1:nsubd,1:ntor)
  call MPI_ALLREDUCE(sbufx,rbufx,nsubd*ntor,  &
       MPI_REAL8,MPI_SUM,  &
       MPI_COMM_WORLD,ierr)
  do k = 1,nsubd
     efl_em(ns,k,:)=rbufx(k,:)/( float(tmm(1)) )*totvol/vol(k)*cn0s(ns)
  end do

  sbufx(1:nsubd,1:ntor) = mypfl_es(1:nsubd,1:ntor)
  call MPI_ALLREDUCE(sbufx,rbufx,nsubd*ntor,  &
       MPI_REAL8,MPI_SUM,  &
       MPI_COMM_WORLD,ierr)
  do k = 1,nsubd
     pfl_es(ns,k,:)=rbufx(k,:)/( float(tmm(1)) )*totvol/vol(k)*cn0s(ns)
  end do

  sbufx(1:nsubd,1:ntor) = mypfl_em(1:nsubd,1:ntor)
  call MPI_ALLREDUCE(sbufx,rbufx,nsubd*ntor,  &
       MPI_REAL8,MPI_SUM,  &
       MPI_COMM_WORLD,ierr)
  do k = 1,nsubd
     pfl_em(ns,k,:)=rbufx(k,:)/( float(tmm(1)) )*totvol/vol(k)*cn0s(ns)
  end do
...
!$acc end atomic
     ...
     !$acc end data
#endif     
     !     !$acc update host(myden,myjpar,mydt)   !error: myden not found on device
...
grid11_end_tm=grid11_end_tm+MPI_WTIME()
     ...
     do i = 0,im
        dtiz(ns,i) = (dtz(i)-gt0s(ns,i)*dnz(i))/(cn0s(ns)*gn0s(ns,i))
     end do
...
!$omp private(r,th,bfldp,b,vfac,dv,dum) &  
!$omp shared(mme,x3e,y3e,z3e,u3e,mue3,w3e,lx,lr0,delz,thfnz,rin,dr,pi,bfld,tor,emass,dx,dy,dz,dth,kcnt,apar,qel,ipara,gclr) &
!$omp shared(w000,w001,w010,w011,w100,w101,w110,w111) &
!$omp shared(nr2,q0p,itube) &
...
!       MPI_SUM,GRID_COMM,ierr)
  ...
  do i = 0,im
     dtez(i) = (dtz(i)-gt0e(i)*dnz(i))/(cn0e*gn0e(i))
  end do
...
subroutine restart(iflag,n)
  ...
  if(idg==1)then
     do i = 0,last
        if(myid==i)then
           open(9, file='plot',status='unknown',position='append')
           write(9,*)'enter restart',myid,iflag,n
           close(9)
        end if
        call MPI_BARRIER(MPI_COMM_WORLD,ierr)
     end do
  ...
  if(iflag.eq.1) then
     ...
     do ns = 1,nsm
        ...
        do m=1,mm(ns)
           ...
           read(139+MyId) xii(m,ns),z0i(m,ns),pzi(m,ns),eki(m,ns),u0i(m,ns),g0i(m,ns)
           w2(m,ns)=w2(m,ns)/cut
           x3(m,ns)=x2(m,ns)
           y3(m,ns)=y2(m,ns)
           z3(m,ns)=z2(m,ns)
...
#ifdef OPENACC
     !$acc update device(x3,y3,z3,u3,w3,mu,x2,y2,z2,u2,w2,xii,z0i,pzi,eki,u0i,g0i)
#endif
120  continue
     ...
     do  m=1,mme
        ...
        read(139+MyId) xie(m),z0e(m),pze(m),eke(m),mue(m),u0e(m),g0e(m)
        w2e(m)=w2e(m)/cut
        x3e(m)=x2e(m)
        y3e(m)=y2e(m)
        z3e(m)=z2e(m)
...
#ifdef OPENACC
     !$acc update device(x3e,y3e,z3e,u3e,w3e,mue3,x2e,y2e,z2e,u2e,w2e,mue2,ipass,xie,z0e,pze,mue,eke,u0e,g0e)
#endif

     if(ifullfbm==1)then
     read(139+MyId)mmb
     do  m=1,mmb
        read(139+MyId) x2b(m),y2b(m),z2b(m),u2b(m),mub(m),wb(m)
        read(139+MyId) xiib(m),z0ib(m),pzib(m),ekib(m),u0ib(m)
        x3b(m)=x2b(m)
        y3b(m)=y2b(m)
        z3b(m)=z2b(m)
        u3b(m)=u2b(m)
...
#ifdef OPENACC
     !$acc update device(x3b,y3b,z3b,u3b,wb,mub,x2b,y2b,z2b,u2b,xiib,z0ib,pzib,ekib,u0ib)
#endif
     end if
     ...
     do k = 0,1
        do j = 0,jmx
           do i = 0,imx
              read(139+myid)denbeq(i,j,k),jparbeq(i,j,k),jparbeqp(i,j,k),jpexbeq(i,j,k),jpeybeq(i,j,k)
           end do
        end do
     end do
     endif
  ...
  if(iflag.eq.2) then
...
#ifdef OPENACC
     !$acc update host(mu,x2,y2,z2,u2,w2,xii,z0i,pzi,eki,u0i,g0i)
     !$acc update host(x2e,y2e,z2e,u2e,w2e,mue2,ipass,xie,z0e,pze,mue,eke,u0e,g0e)
     !$acc update host(x3b,y3b,z3b,u3b,wb,mub,x2b,y2b,z2b,u2b,xiib,z0ib,pzib,ekib,u0ib)
#endif
     ...
     do ns = 1,nsm
        ...
        do m=1,mm(ns)
           ...
           write(139+MyId) xii(m,ns),z0i(m,ns),pzi(m,ns),eki(m,ns),u0i(m,ns),g0i(m,ns)
        enddo
     end do
     ...
     do  m=1,mme
        ...
        write(139+MyId) xie(m),z0e(m),pze(m),eke(m),mue(m),u0e(m),g0e(m)
     end do

     if(ifullfbm==1)then
     write(139+MyId)mmb
     do  m=1,mmb
        write(139+MyId) x2b(m),y2b(m),z2b(m),u2b(m),mub(m),wb(m)
        write(139+MyId) xiib(m),z0ib(m),pzib(m),ekib(m),u0ib(m)
        x3b(m)=x2b(m)
        y3b(m)=y2b(m)
        z3b(m)=z2b(m)
        u3b(m)=u2b(m)
     ...
     do k = 0,1
        do j = 0,jmx
           do i = 0,imx
              write(139+myid)denbeq(i,j,k),jparbeq(i,j,k),jparbeqp(i,j,k),jpexbeq(i,j,k),jpeybeq(i,j,k)
           end do
        end do
     end do
     endif
  ...
  if(idg==1)then
     do i = 0,last
        if(myid==i)then
           open(9, file='plot',status='unknown',position='append')
           write(9,*)'end of restart',myid,iflag,n
           close(9)
        end if
        call MPI_BARRIER(MPI_COMM_WORLD,ierr)
     end do
...
subroutine weight
  ...
  if (GCLR.eq.Master.and.peritr.eq.0) then
     do i=0,im
        ...
        wx0 = (rin+(j+1)*dr-r)/dr
        wx1 = 1.-wx0
        if(itube==0)qr = wx0*sf(j)+wx1*sf(j+1)
        if(itube==1)qr = q0+q0p*(r-lr0)
...
subroutine spec(n)
  ...
  do j = 1+k,nsubd-k
     do l = 1,ntor
     pf = pf+pfle_es(j,l)*vol(j)/totvol*x
     efe = efe+efle_es(j,l)*vol(j)/totvol*x
     efi = efi+efl_es(1,j,l)*vol(j)/totvol*x
     pfi = pfi+pfl_es(1,j,l)*vol(j)/totvol*x
     efc = efc+efl_es(2,j,l)*vol(j)/totvol*x
     pfc = pfc+pfl_es(2,j,l)*vol(j)/totvol*x
     !     efb = efb+eflb_es(j,l)*vol(j)/totvol*x
     !     pfb = pfb+pflb_es(j,l)*vol(j)/totvol*x
     end do
  end do

  do j = 1+k,nsubd-k
     do l = 1,ntor
     pf_em = pf_em+pfle_em(j,l)*vol(j)/totvol*x
     efe_em = efe_em+efle_em(j,l)*vol(j)/totvol*x
     efi_em = efi_em+efl_em(1,j,l)*vol(j)/totvol*x
     pfi_em = pfi_em+pfl_em(1,j,l)*vol(j)/totvol*x
     efc_em = efc_em+efl_em(2,j,l)*vol(j)/totvol*x
     pfc_em = pfc_em+pfl_em(2,j,l)*vol(j)/totvol*x
     !     efb_em = efb_em+eflb_em(j,l)*vol(j)/totvol*x
     !     pfb_em = pfb_em+pflb_em(j,l)*vol(j)/totvol*x
     end do
  end do
  ...
  if(myid.eq.master)then
     ...
     write(*,10)tdum,rmsphi(n),rmsapa(n),pf,efe,pfi,efi,avewi(1,n),avewb(n),&
...
15   format(1x,f10.1,8(2x,e10.3))
     ...
     write(12,17)tdum,((pfle_es(i,l)/pflxgb,i=1,nsubd),l=1,ntor)
     write(12,17)tdum,((pfl_es(1,i,l)/pflxgb,i=1,nsubd),l=1,ntor) 
     write(12,17)tdum,((pfl_es(2,i,l)/pflxgb,i=1,nsubd),l=1,ntor)
     write(12,17)tdum,((efle_es(i,l)/eflxgb,i=1,nsubd),l=1,ntor)
     write(12,17)tdum,((efl_es(1,i,l)/eflxgb,i=1,nsubd),l=1,ntor)
     write(12,17)tdum,((efl_es(2,i,l)/eflxgb,i=1,nsubd),l=1,ntor)
     write(12,17)tdum,((pfle_em(i,l)/pflxgb,i=1,nsubd),l=1,ntor)
     write(12,17)tdum,((pfl_em(1,i,l)/pflxgb,i=1,nsubd),l=1,ntor) 
     write(12,17)tdum,((pfl_em(2,i,l)/pflxgb,i=1,nsubd),l=1,ntor)
     write(12,17)tdum,((efle_em(i,l)/eflxgb,i=1,nsubd),l=1,ntor)
     write(12,17)tdum,((efl_em(1,i,l)/eflxgb,i=1,nsubd),l=1,ntor)
     write(12,17)tdum,((efl_em(2,i,l)/eflxgb,i=1,nsubd),l=1,ntor)
     
     write(17,12)tdum,yyre(1,0),yyim(1,0),yyre(1,1),yyim(1,1),yyre(1,2),yyim(1,2),yyre(1,3),yyim(1,3),yyre(1,4),yyim(1,4)
     close(9)
     close(11)
     close(12)     
     close(17)
...
subroutine loadi(ns)
  ...
  do j = 1,100000000
     ...
     if(ran2(iseed)<(0.5*jacp/jacmax))then
        ...
        xii(m,ns) = x2(m,ns)
        u0i(m,ns) = u2(m,ns)
        g0i(m,ns) = 1./(2*pi*ter)**1.5*exp(-eki(m,ns)/ter) *mims(ns)**1.5
        myavgv=myavgv+u2(m,ns)

        !    LINEAR: perturb w(ns,m) to get linear growth...
!        w2(ns,m)=2.*amp*(revers(MyId*cnt+m,13)-0.5) !(ran2(iseed) - 0.5 )
        w2(m,ns)=1.*amp*sin(1*pi2/ly*y2(m,ns))*exp(-(z2(m,ns)-lz/2)**2/(lz/8)**2)*exp(-(x2(m,ns)-0.5*lx)**2/(lx/8)**2) &
                +2.*amp*sin(2*pi2/ly*y2(m,ns))*exp(-(z2(m,ns)-lz/2)**2/(lz/8)**2)*exp(-(x2(m,ns)-0.5*lx)**2/(lx/8)**2) &
                +3.*amp*sin(3*pi2/ly*y2(m,ns))*exp(-(z2(m,ns)-lz/2)**2/(lz/8)**2)*exp(-(x2(m,ns)-0.5*lx)**2/(lx/8)**2) &
                +4.*amp*sin(4*pi2/ly*y2(m,ns))*exp(-(z2(m,ns)-lz/2)**2/(lz/8)**2)*exp(-(x2(m,ns)-0.5*lx)**2/(lx/8)**2)
        if(ns==2)w2(m,ns) = 0.
...
170 continue
  ...
  call pmove(g0i(:,ns),np_old,np_new,ierr)
  if (ierr.ne.0) call ppexit
  !$acc update device(g0i(:,ns))
#endif
...
subroutine pint
  ...
  INTEGER :: np_old,np_new
  real :: vfac,kap,kapnp,kaptp,sz,vpar,ppar,vpdum,pidum,xnp,omrotp
...
!$omp private(xdt0,ydt0,xdtes,ydtes,xdtem,ydtem,omrotp) &
!$omp private(fdum,gdum,fesrcp,dnesrcp,avwexepsp,fovg,dtp,avwexezp,dnesrczp) &  
!$omp private(dapsdtp,delbxhp,delbyhp,dahdzp,aparhp,zdot0,zdot1) &    !MVPT

!$omp shared(x2e,y2e,z2e,u2e,w2e,mue,mue2,mue3,x3e,y3e,z3e,u3e,w3e,g0e,eke,pze,xie,z0e,u0e,gmrkre,avwexeps,avwexez,dnesrc,dnesrcz,fesrc,dtez) &
!$omp shared(ex,ey,ez,delbx,delby,apar,phi,dpdz,dadz,delbxh,delbyh,dahdz,aparh,dphidt) &
!$omp shared(thfnz,dbdr,dbdth,grcgt,bfld,radius,dydr,qhat,gr,gxdgy,curvbz,bdcrvb,grdgt,f,sf,jfn,psip,psi,t0e,capte,capne,xn0e) &
!$omp shared(phincp,vparsp,psip2,dipdr,omrot) &
!$omp shared(lr0,r0,rin,pi,br0,tor,q0,lx,ly,lz,dx,dy,dz,delz,dr,dth,dxsrc,desrc,nesrc,nxsrc,nr,gclr,tclr,kcnt,vexbsw,vparsw) &
!$omp shared(ipbm,itube,peritr,iperidf,isonew,gammah,gamgtc,ghzon,gamtoy,gamgyro,nsubd) &
!$omp shared(mme,emass,qel,nonline,dte,eldu,tge,ig0e,igmrkre,pzcrite,im) &
!$omp shared(nue0,zeff,ipara,iexb,rneu,isunie,cv,isg,iflut) &
!$omp shared(nr2,q0p) &  
...
!$acc loop gang vector
  ...
  do m=1,mme
     ...
     !MVPT
     if(ipbm==1)then
        edot = qel*((xdt0+xdtem*nonline)*exp1+(ydt0+ydtem*nonline-vp0)*eyp+zdot1*ezp) &
             +qel*pzd0*aparhp &
             +qel*vpdum*(-(xdt0+xdtes*nonline)*delbyhp+(ydt0+ydtes*nonline)*delbxhp+zdot*dahdzp)   &
             -qel*vpar*delbxhp*vp0
     end if
...
!$acc end parallel
  ...
  !!$acc update host(z3e,x2e,x3e,y2e,y3e,z2e,u2e,u3e,w2e,w3e,mue2,mue3,index,ipass,mue,xie,z0e,eke,pze,u0e,g0e)
  np_old=mme
  initpmove_start_tm=initpmove_start_tm+MPI_WTIME()
  ...
  if (ierr.ne.0) call ppexit
  call test_pmove(g0e,np_old,np_new,ierr)
  if (ierr.ne.0) call ppexit
  pmove_end_tm=pmove_end_tm+MPI_WTIME()  
  call end_pmove(ierr)
  mme=np_new
  !!$acc update device(z3e,x2e,x3e,y2e,y3e,z2e,u2e,u3e,w2e,w3e,mue2,mue3,index,ipass,mue,xie,z0e,eke,pze,u0e,g0e)
  !      return
end subroutine pint
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
subroutine cint(n)
  ...
  real :: phip,exp1,eyp,ezp,delbxp,delbyp,dgdtp,dpdzp,dadzp,aparp,vncp
  real :: dum,vxdum,dum1,dum2,eps,x,h_x,h_coll,hee,nue
  INTEGER :: m,i,j,k,k1,l,n,mynowe,mynovpar
  INTEGER :: np_old,np_new
  real :: vfac,kap,kapnp,kaptp,sz,vpar,ppar,vpdum,pidum,xnp,omrotp
  real :: b,th,r,enerb,cost,sint,qr,laps,ter,dtp
  real :: xt,xs,yt,zt,xdot,ydot,zdot,pzdot,edot,pzd0,pzd1,vp0,xdt0,ydt0,xdtes,ydtes,xdtem,ydtem
  real :: wx0,wx1,wy0,wy1,wz0,wz1,w3old,z3old
  real :: myke,mypfl_es(1:nsubd,1:ntor),mypfl_em(1:nsubd,1:ntor),myptrp
  real :: myefl_es(1:nsubd,1:ntor),myefl_em(1:nsubd,1:ntor),mynos,myavewe
...
#ifdef OPENMP  
!$omp parallel do default(none) &
!$omp private(i,j,k,k1,r,th,wx0,wx1,wy0,wy1,wz0,wz1,dbdrp,dbdtp,grcgtp,bfldp,radiusp,dydrp,qhatp,grp,gxdgyp,curvbzp,bdcrvbp,grdgtp) &
!$omp private(fp,jfnp,psipp,psp,ter,kaptp,kapnp,xnp,vncp,psip2p,dipdrp,nue0p,zeffp,b,pzp,xt,yt,x000,x001,x010,x011,x100,x101,x110,x111) &
!$omp private(exp1,eyp,ezp,delbxp,delbyp,dgdtp,dpdzp,dadzp,aparp,phip,vfac,vp0,kap,ppar,vpar,bstar,enerb,dum2,vxdum,xdot,ydot,zdot) &
!$omp private(pzd0,pzd1,pzdot,vpdum,edot,eps,x,h_x,h_coll,hee,nue,dum1,dum,laps,qr,w3old,z3old) &
!$omp private(xdt0,ydt0,xdtes,ydtes,xdtem,ydtem,omrotp) &  
!$omp private(fdum,gdum,fesrcp,dnesrcp,avwexepsp,fovg,dtp,avwexezp,dnesrczp) &
!$omp private(dapsdtp,delbxhp,delbyhp,dahdzp,aparhp,zdot0,zdot1) &    !MVPT  

!$omp shared(x2e,y2e,z2e,u2e,w2e,mue,mue2,mue3,x3e,y3e,z3e,u3e,w3e,g0e,eke,pze,xie,z0e,u0e,gmrkre,avwexeps,avwexez,dnesrc,dnesrcz,fesrc,dtez) &
!$omp shared(ex,ey,ez,delbx,delby,apar,phi,dpdz,dadz,delbxh,delbyh,dahdz,aparh,dphidt,eyky,dbxky) &
!$omp shared(thfnz,dbdr,dbdth,grcgt,bfld,radius,dydr,qhat,gr,gxdgy,curvbz,bdcrvb,grdgt,f,sf,jfn,psip,psi,t0e,capte,capne,xn0e) &
!$omp shared(phincp,vparsp,psip2,dipdr,omrot) &
!$omp shared(lr0,r0,rin,pi,br0,tor,q0,lx,ly,lz,dx,dy,dz,delz,dr,dth,dxsrc,desrc,nesrc,nxsrc,nr,gclr,tclr,kcnt,vexbsw,vparsw) &
!$omp shared(ipbm,itube,peritr,iperidf,isonew,gammah,gamgtc,ghzon,gamtoy,gamgyro,nsubd,ntor) &
!$omp shared(mme,emass,qel,nonline,dte,eldu,tge,ig0e,igmrkre,pzcrite,im,wecut,vsphere,ipass) &
!$omp shared(nue0,zeff,ipara,iexb,rneu,isunie,cv,isg,iflut) &
!$omp shared(nr2,q0p) &
!$omp reduction(+: myavewe,myke,mypfl_es,mypfl_em,myptrp,myefl_es,myefl_em,mynos,mytotn,mytrap,mynowe,mynovpar)
#endif
...
!        w2e(m) = 0.
     ...
     i = int(x3e(m)/dx+0.5)
     j = int(y3e(m)/dy+0.5)
     k1=int(z3old/dz+0.5)-gclr
#ifdef OPENACC
!$acc loop seq     
     do l = 1,ntor
     !$acc atomic     
     mypfl_es(k,l)=mypfl_es(k,l) + w3old*(eyky(i,j,k1,l)/b)*dum2/grp
     !$acc atomic     
     mypfl_em(k,l)=mypfl_em(k,l) + w3old*(vpar*dbxky(i,j,k1,l)/b)*dum2/grp
     !$acc atomic          
     myefl_es(k,l)=myefl_es(k,l) + vfac*w3old*(eyky(i,j,k1,l)/b)*dum2/grp
     !$acc atomic     
     myefl_em(k,l)=myefl_em(k,l) + vfac*w3old*(vpar*dbxky(i,j,k1,l)/b)*dum2/grp
     end do
     !$acc atomic     
...
!$acc end parallel
  ...
  call MPI_ALLREDUCE(sbufx,rbufx,nsubd*ntor,  &
       ...
       MPI_COMM_WORLD,ierr)
  do k = 1,nsubd   
     efle_es(k,1:ntor)=rbufx(k,1:ntor)/( float(tmm(2)) )*totvol/vol(k)*cn0e
  end do
  ...
  call MPI_ALLREDUCE(sbufx,rbufx,nsubd*ntor,  &
       ...
       MPI_COMM_WORLD,ierr)
  do k = 1,nsubd   
     efle_em(k,1:ntor)=rbufx(k,1:ntor)/( float(tmm(2)) )*totvol/vol(k)*cn0e
  end do
  ...
  call MPI_ALLREDUCE(sbufx,rbufx,nsubd*ntor,  &
       ...
       MPI_COMM_WORLD,ierr)
  do k = 1,nsubd   
     pfle_es(k,1:ntor)=rbufx(k,1:ntor)/( float(tmm(2)) )*totvol/vol(k)*cn0e
  end do
  ...
  call MPI_ALLREDUCE(sbufx,rbufx,nsubd*ntor,  &
       ...
       MPI_COMM_WORLD,ierr)
  do k = 1,nsubd   
     pfle_em(k,1:ntor)=rbufx(k,1:ntor)/( float(tmm(2)) )*totvol/vol(k)*cn0e
  end do
...
subroutine jie(ip,n)
  ...
  real :: kapnp,kaptp,xnp,psip2p,bdcrvbp,curvbzp,dipdrp,bstar,omrotp
...
!$omp private(psipp,ter,kaptp,kapnp,xnp,vncp,vparspp,psip2p,dipdrp,b,rhog,vfac,vp0,vpar,kap,wght,wght0,wght1,rhox,rhoy,omrotp) &
...
!$omp shared(ipbm,itube,peritr,iperidf,isonew,gammah,gamgtc,ghzon,gamtoy,gamgyro,nsubd) &
!$omp shared(mm,ns,mims,iflr,iorb,lr,nonlin,dt,ildu,tgis,ig0,igmrkr,pzcrit) &
!$omp shared(e1gx,e1gy,e2gx,e2gy,bdge1gx,bdge1gy,bdge2gx,bdge2gy,isuni,cn,sn) &
!$omp shared(nr2,q0p) &     
...
!$acc parallel
!$acc loop gang vector private(rhox,rhoy) private(gdum)
#endif
     do m=1,mm(ns)
...
!$omp parallel do default(none) &
!$omp private(i,j,k,l,dv,r,th,wx0,wx1,wz0,wz1,dbdrp,dbdtp,grcgtp,bfldp,radiusp,dydrp,qhatp,grp,gxdgyp,curvbzp,bdcrvbp,grdgtp,fp,jfnp) &
!$omp private(psipp,ter,kaptp,kapnp,xnp,vncp,vparspp,psip2p,dipdrp,b,rhog,vfac,vp0,vpar,kap,wght,wght0,wght1,wght2,rhox,rhoy,omrotp) &
...
!$acc loop gang vector private(gdum)
  ...
  do m=1,mme
     ...
     vpar = u3e(m) !-qel/emass*aparp*ipara !aparp not assigned yet
     vfac = 0.5*(emass*u3e(m)**2 + 2.*mue3(m)*b)
     vp0 = r0/q0*omrotp*vexbsw !1./b**2*lr0/q0*qhatp*fp/radiusp*grcgtp
     kap = kapnp - (1.5-vfac/ter)*kaptp
     ...
     dum1 = 1./b*lr0/q0*qhatp*fp/radiusp*grcgtp
     !Here iflut should appear in vxdum as it is used for the divergence of (\int df * v_G). Later, iflut is not needed in dnedt       
     ...
     wght=w3e(m)/dv
     wght0=1./dv
     wght1 = (wght+1./dv*phip*isg/ter*fovg-1./dv*qel/ter*vpar*aparp*fovg)
     wght2 = wght*zdot
#ifdef OPENACC
     !$acc atomic
...
subroutine lorentz(ip,n)
  ...
  integer :: i,j,ip,k,m,n,ncol,icol,npool
  ...
  !$omp private(i,j,k,icol,r,th,wx0,wx1,wy0,wy1,wz0,wz1,ter,nuep,zeffp,xnp,bfldp,b,vfac,fdum,gdum,fovg,xt,yt,zt,x000,x001,x010,x011) &
  !$omp private(x100,x101,x110,x111,aparp,uold,edum,vdum,ptch,eps,x,h_x,h_coll,hee,nue,dum1,dum) &
  !$omp private(rndm,wtmp)
#endif
...
!perform random kick 
...
#endif
     do icol = 1,ncol
        rndm = randpool(modulo((m-1)*ncol+icol,npool))
     ...
     wtmp = w3e(m)-qel/ter*uold*aparp*fovg 
     w3e(m) = wtmp+qel/ter*u3e(m)*aparp*fovg 
     if(ip==0)w2e(m) = w3e(m)
...
subroutine loader_wrapper
  ...
  if(ifullfbm==1)call loadb
  if(idg==1)then
     do i = 0,last
        if(myid==i)then
           open(9, file='plot',status='unknown',position='append')
           write(9,*)'pass beam loader',myid
           close(9)
        end if
        call MPI_BARRIER(MPI_COMM_WORLD,ierr)
     end do
  ...
  if(idg==1)then
     do i = 0,last
        if(myid==i)then
           open(9, file='plot',status='unknown',position='append')
           write(9,*)'pass electron loader',myid
           close(9)
        end if
        call MPI_BARRIER(MPI_COMM_WORLD,ierr)
     end do
...
!  if(myid==0)write(*,*)'before grid1',MPI_WTIME()
  grid1_start_tm0=grid1_start_tm0+MPI_WTIME()
  call grid1(ip,n)
  if(ifullfbm==1)call grid1b(ip,n)  
  grid1_end_tm0=grid1_end_tm0+MPI_WTIME()
  ...
  do it = 1,iter
     ...
     if(idg==1)then
        do i = 0,last
           if(myid==i)then
              open(9, file='plot',status='unknown',position='append')
              write(9,*)'pass gkps in poisson',myid
              close(9)
           end if
           call MPI_BARRIER(MPI_COMM_WORLD,ierr)
        end do
...
subroutine split_weight(n,ip)
  ...
  if(idg==1)then
     do i = 0,last
        if(myid==i)then
           open(9, file='plot',status='unknown',position='append')
           write(9,*)'pass split_weight',myid
           close(9)
        end if
        call MPI_BARRIER(MPI_COMM_WORLD,ierr)
     end do
...
subroutine push_wrapper(n,ip)
  ...
  if(idg==1)then
     do i = 0,last
        if(myid==i)then
           open(9, file='plot',status='unknown',position='append')
           write(9,*)'enter push_wrapper',myid
           close(9)
        end if
        call MPI_BARRIER(MPI_COMM_WORLD,ierr)
     end do
  ...
  if(ip==1 .and. ifullfbm==1)call ppushb(n)
  if(ip==0 .and. ifullfbm==1)call cpushb(n)
...
!$omp shared(mme,lx,q0,lr0,br0,mue3,x3e,y3e,z3e,u3e,apar,dx,dy,dz,delz,dr,dth,qel,emass,ipara,thfnz,rin,t0e,xn0e,tor,itp) &
!$omp shared(grcgt,radius,bfld,dbdr,jfn,psip,f,pi,dxsrc,nxsrc,desrc,nesrc,igmrkre,gmrkre,ig0e,g0e,amie,eldu,isunie,cv,tge) &
!$omp shared(w000,w001,w010,w011,w100,w101,w110,w111) &
!$omp shared(nr2,q0p,itube) &  
...
subroutine ldel
  ...
  do j = 1,100000000
     ...
     if(ran2(iseed)<(0.5*jacp/jacmax))then
        ...
        xie(m) = x2e(m)
        u0e(m) = u2e(m)
        g0e(m) = 1./(2*pi*ter)**1.5*exp(-eke(m)/ter) *emass**1.5
        myavgv=myavgv+u2e(m)
...
170 continue
  ...
  call pmove(g0e,np_old,np_new,ierr)
  if (ierr.ne.0) call ppexit
  !$acc update device(g0e(:))
#endif
        ...
        do m=0,jm-1
           do n = 0,km
              ...
              if(b(1).lt.3.e-5)then
                 ...
                 gamphi = 0.
                 do ns = 1, nsm
                    gamphi = gamphi + q(ns)*q(ns)*cn0s(ns)*gn0s(ns,l)/ter(ns)*(1-gam0(ns))
                 enddo
...
subroutine dcmpky(u,v)
  ...
  INTEGER :: i,j,k,k1,l,m,n,ifirst,nstep
  complex :: temp3dxy(0:imx-1,0:jmx-1,0:1)
  real :: u(0:imx,0:jmx,0:1),v(0:imx,0:jmx,0:1,0:ntor)
  real :: dum,dum1
  ...
  do k = 0,1
     do i = 0,imx-1
        do j = 0,jmx-1
           v(i,j,k,0) = temp3dxy(i,0,k)/jmx
           do n = 1,ntor
              v(i,j,k,n) = (temp3dxy(i,n,k)*exp(iu*pi2*n*j/float(jmx))+temp3dxy(i,jmx-n,k)*exp(-iu*pi2*n*j/float(jmx)))/jmx
           end do
        end do
     end do
  end do
        ...
        do j = 0,jcnt-1
           do i = 1,imx-1
              ...
              do ix = 1,imx-1
                 ...
                 do ikx = 0,imx-1
                    do ns = 1, nsm
                       mxg(i,ix,j,k) = mxg(i,ix,j,k)+q(ns)*q(ns)*sin(ix*ikx*pi/imx)* &
...
do i = 1,imx-1
   do m = 0,jcnt-1
      do k = 0,1
         write(20000+MyId)(mxa(i,j,m,k),j=1,imx-1),ipiva(i,m,k)
      end do
   end do
 end do
 close(20000+myid)
...
if(igetmx.eq.1) then
 ...
 do i = 1,imx-1
    do m = 0,jcnt-1
       do k = 0,1
          read(20000+MyId)(mxa(i,j,m,k),j=1,imx-1),ipiva(i,m,k)
       end do
    end do
 end do
 close(20000+myid)
        ...
        do j = 0,jcnt-1
           do i = 1,imx-1
              ...
              do ix = 1,imx-1
                 ...
                 do ikx = 0,imx-1
                    do ns = 1, nsm
                       mxd(i,ix,j,k) = mxd(i,ix,j,k)+q(ns)*q(ns)*sin(ix*ikx*pi/imx)* &
                            ((1-gamb1(ns,ikx,j,i,k))*exp(IU*ikx*i*pi/imx)- &
                            (1-gamb2(ns,ikx,j,i,k))*exp(-IU*ikx*i*pi/imx)) &
                            /ter(ns)*cn0s(ns)*gn0s(ns,i)/(IU*imx)
                    end do
...
open(30000+MyId,file=fname,form='unformatted',status='unknown')
 do i = 1,imx-1
    do m = 0,jcnt-1
       do k = 0,1
          write(30000+MyId)(mxd(i,j,m,k),j=1,imx-1),ipivd(i,m,k)
       end do
    end do
 end do
 close(30000+myid)
...
if(igetmx==1)then
 ...
 do i = 1,imx-1
    do m = 0,jcnt-1
       do k = 0,1
          read(30000+MyId)(mxd(i,j,m,k),j=1,imx-1),ipivd(i,m,k)
       end do
    end do
 end do
 close(30000+myid)
...
!$omp parallel do default(none) &
  ...
  !$omp shared(mme,x3e,z3e,lx,lr0,delz,thfnz,dr,dth,pi,rin,bfld,t0e,xn0e,emass,u3e,mue3,dxsrc,desrc,nxsrc,nesrc,w3e) &
  !$omp shared(nr2,q0p,itube) &
  !$omp reduction(+: myg,myavw)
#endif
...
11   format(1x,i15)
  ...
  do m = 0, nxsrc-1
     ...
     do j = 0, nesrc-1
        ...
        gdum = 1.0/(2*pi*ter)**1.5*exp(-vfac/ter) * emass**1.5
        if(eldu==1)gdum = 1./(2*pi*tge)**1.5*exp(-vfac/tge) *emass**1.5
        fdum = xn/(2*pi*ter)**1.5*exp(-vfac/ter) * emass**1.5
        dn = dn+avwexeps(m,j)*gmrkre(m,j) * 4*pi*v**2 *dv
...
subroutine wiatxeps(ns,igtilde)
  ...
  real :: am(0:nesrc-1,1:nzgyro)
  real :: myjac(0:imx),totjac(0:imx)
  real :: nclambda(0:19,0:63),mynclambda(0:19,0:63),sloc,eloc,dsloc,deloc,dlambda,ptch
  integer :: nlambda,iptch,isitin
...
#ifdef OPENMP
!$omp parallel do &
!$omp private(i,k,ie,r,th,wx0,wx1,wz0,wz1,b,ter,xnp,vfac,xt,ptch,isitin,iptch) &
  !$omp shared(mm,ns,x3,z3,lx,lr0,delz,thfnz,dr,dth,pi,rin,bfld,t0e,xn0e,mims,u3,mu,dxsrc,desrc,nxsrc,nesrc,w3) &
  !$omp reduction(+: myg,myavw,mynclambda)
#endif
  ...
  do m=1,mm(ns)
     ...
     ptch = u3(m,ns)/sqrt(2*vfac*ter/mims(ns))
     isitin=0
     if(abs(r/a-sloc)<dsloc .and. abs(vfac-eloc)<deloc)isitin=1
     
     xt = x3(m,ns)
     i=int(xt/dxsrc)
     i = min(i,nxsrc-1)
     ie = int(vfac/desrc)
     iptch = int((ptch+1.0)/dlambda)
...
!$acc end atomic     
!$acc atomic update     
     mynclambda(iptch,gclr) = mynclambda(iptch,gclr)+isitin /jac(int(x3(m,ns)/dx),0)
!$acc end atomic
#endif
...
!$omp end parallel do
  ...
  call MPI_ALLREDUCE(myg,  &
       ...
       bfcnt,MPI_REAL8,       &
       MPI_SUM,MPI_COMM_WORLD,ierr)
  call MPI_ALLREDUCE(mynclambda,  &
       nclambda,             &
       nlambda*kmx,MPI_REAL8,       &
       MPI_SUM,MPI_COMM_WORLD,ierr)
...
!g diagnostics, gtilde=g/volume(x,eps,dx,deps)
  if(igtilde==1)then
     if(myid.eq.master)then
        ...
        open(9, file='ptcnt', status='unknown',position='rewind')
        do k = 0,kmx-1
           dum = 0.
           do i = 0,nlambda-1
              dum = dum+nclambda(i,k)
              write(9,10)k,i,nclambda(i,k)
           end do
           write(9,*)int(dum), '  gclr'
        enddo
        close(9)
...
11   format(1x,i15)
  ...
  do m = 0, nxsrc-1
     ...
     do j = 0, nesrc-1
        ...
        gdum = 1.0/(2*pi*ter)**1.5*exp(-vfac/ter) *mims(ns)**1.5
        if(ildu==1)gdum = 1./(2*pi*tgis(ns))**1.5*exp(-vfac/tgis(ns)) *mims(ns)**1.5
        fdum = xn/(2*pi*ter)**1.5*exp(-vfac/ter) *mims(ns)**1.5
        dn = dn+avwixeps(ns,m,j)*gmrkr(ns,m,j) * 4*pi*v**2*dv
        totn = totn+fdum * 4*pi*v**2*dv
     end do
     fisrc(ns,m) = totn
     dnisrc(ns,m) = dn

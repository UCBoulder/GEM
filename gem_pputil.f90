...
#else
  PUBLIC :: ppinit_mpi,ppinit_decomp,ppexit,init_pmove,end_pmove,pmove,guard,test_pmove,test_init_pmove
#endif
...
!
  ...
  INTEGER, DIMENSION(:), ALLOCATABLE, SAVE ::s_counts,s_displ,s_counts_before,s_counts_after
...
CONTAINS
  ...
  SUBROUTINE test_init_pmove(xp, np, lz, ierr)
    ...
    INTEGER :: i, ip, iz, ih, iwork,icounts
    !!!for ipfill (htc)
    ...
    ALLOCATE(iz_arr(1:xpsize*4))  !first called on ions, but electrons can have 4x particle number
#ifdef OPENACC    
    ...
    IF( .not. ALLOCATED(s_buf) ) THEN
       ksize=xpsize         !memory is abundant for GEM
...
!    !$acc end serial
!    !$acc end data
    !$acc update device(s_counts,iphole)
#endif
...
#endif
    do ip=0,nvp-1
       nsize=nsize+s_counts(ip)
    enddo
#ifdef OPENACC    
!    !$acc end data
#endif

!    write(*,*)'i is not equal to nsize', GCLR,i, nsize
...
#ifdef OPENACC
    !$acc update host(s_displ)
    !$acc wait
#endif

    !
    !if(me==0)write(*,*)'after test pmove 1.4',mpi_wtime()
    !----------------------------------------------------------------------
    ...
    do ip = 1,nsize
       iz=iz_arr(iphole(ip))
       isb(iz) = isb(iz)+1
       ipsend(isb(iz)) = iphole(ip)
    end do

#ifdef OPENACC    
    !$acc update device(ipsend(1:nsize))
#endif

    np_new_ipf=np-nsize
    ip_last_ipf=np

    ipfill(1:nsize) = iphole(1:nsize)
    
#ifdef OPENACC
!    !$acc serial present(xp,ipfill,iphole,iz_arr)
!    !$acc update host(ipfill)
#endif

    ih_ipf = 0
    DO i=np,np-nsize+1,-1
       iz=iz_arr(i)
       IF (iz==GCLR) THEN
          ih_ipf = ih_ipf+1
          ipfill(ih_ipf)=i
...
#ifdef OPENACC    
!    !$acc end serial
!$acc update device(ipfill(1:nsize))
#endif

#ifdef OPENMP
#endif

    !if(me==0)write(*,*)'after test pmove 2.1',mpi_wtime()
    ...
    !    call ppsum(ierr)
    !if(me==0)write(*,*)'after test pmove 3',mpi_wtime()
    !----------------------------------------------------------------------
    !              4.  Initialize tag
    !
    pmove_tag = 101
...
!  Local arrays
  ...
  !!is_present is used to test the data is on device or not, using
  !acc_is_present
...
#ifdef OPENACC
!$acc parallel present(s_counts)
    s_buf_length=0
!$acc loop gang vector reduction(+:s_buf_length) 
    do i=0,nvp-1
...
!===========================================================================
  ...
  SUBROUTINE init_pmove(xp, np, lz, ierr)
    ...
    if(me==0)write(*,*)'init_pmove 1',MPI_WTIME()
    ...
    !              1.  Construct send buffer
    !
    if(me==0)write(*,*)'init_pmove 2',MPI_WTIME()
    dzz = lz / nvp
    s_counts = 0
    DO ip = 1,np
       xt = MODULO(xp(ip), lz)            !!! Assume periodicity
    ...
    END IF
    !
    if(me==0)write(*,*)'init_pmove 3',MPI_WTIME()
    !----------------------------------------------------------------------
    !              2.  Construct (sorted) pointers to holes
    !
    isb(0:nvp-1) = s_displ(0:nvp-1)
    ...
    END DO
    !
    if(me==0)write(*,*)'init_pmove 4',MPI_WTIME()
    !----------------------------------------------------------------------
    !              3.  Construct receive buffer
    !
    CALL MPI_ALLTOALL(s_counts, 1, MPI_INTEGER, &
    ...
    ierr = 0
    nsize = np - sum(s_counts) + sum(r_counts)
    !write(*,*) 'PE',me,nsize,size(xp),np,sum(s_counts),sum(r_counts)
    if( nsize .gt. size(xp) ) then
       write(*,*) 'PE', me, 'Particle array overflow',nsize, size(xp)
       ierr = 1
    end if
    !    call ppsum(ierr)
    !
    ...
    !              4.  Initialize tag
    !
    if(me==0)write(*,*)'init_pmove 5',MPI_WTIME()
    pmove_tag = 101
    !
  END SUBROUTINE init_pmove
#endif

...
readf,1, t,x0,x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11
t0(k) = t
y5(0,k) = x0
y5(1,k) = x1
y5(2,k) = x2
...
t0 = t0*csovrmaj
xmax = max([max(t0)])
ymax = max([   max(abs(y5(ip,0:n5-1)))   ])
ymin = min([   min(y5(ip,0:n5-1))   ])
plot, t0(0:n5-1),y5(ip,0:n5-1), title='', $
  xstyle=1, ystyle=1,xtitle='!3tC!ls!N/R',$
  ytitle='Q!li',$ 
  xrange=[0,xmax*1.1], $
  yrange=[-ymax*0.1,ymax*1.2],linestyle=0,color=0,$
  charsize=2,charthick=4,position=[0.3,0.3,0.9,0.9]

;oplot,t0(0:n5-1),y5(11,0:n5-1),linestyle=0,color=lu
;oplot,t0(0:n5-1),y5(ip,0:n5-1)+y5(11,0:n5-1),linestyle=1,color=hong
